    ...
    <section id="income" className="py-20 lg:py-28">
      <div className="container mx-auto px-4 lg:px-8">
        <SectionHeader
          ...
          subtitle="Два прозрачных тарифа — выбирайте подходящий формат работы."
        />
        <div className="grid gap-6 md:grid-cols-2 max-w-5xl">
          {plans.map(({ icon: Icon, name, desc, price, features, highlight }) => (
            <Card
              key={name}
              className={`relative p-8 ${highlight ? "border-gold border-2 shadow-[var(--shadow-elevated)]" : "border-border"}`}
            >
              {highlight && (
                <span className="absolute -top-3 right-6 bg-gold text-gold-foreground text-xs font-bold px-3 py-1 rounded-full">
                  Популярный
                </span>
              )}
              <div
                className={`flex h-12 w-12 items-center justify-center rounded-xl mb-5 ${highlight ? "bg-gold/15 text-gold" : "bg-primary/10 text-primary"}`}
              >
